...
if ! "$PY" -c 'import pypdf' >/dev/null 2>&1; then
  echo "Installing RAG dependency: pypdf"
  "$PIP" install 'pypdf>=5,<7'
fi

EMBED_MODEL="embeddinggemma"
if ! ollama show "$EMBED_MODEL" >/dev/null 2>&1; then
  echo "Downloading RAG embedding model: $EMBED_MODEL"
  ollama pull "$EMBED_MODEL"
else
  echo "RAG embedding model available: $EMBED_MODEL"
fi
...
if [[ ! -f "$DATA/corpus.jsonl" || ! -f "$DATA/queries.jsonl" || ! -f "$DATA/qrels/test.tsv" ]]; then
  echo "Downloading BEIR SciFact dataset for external retrieval validation"
