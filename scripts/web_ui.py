...
function renderPrecheckSteps(output){let steps=new Map;for(let line of output.split('\n')){if(!line.startsWith('@@PRECHECK|'))continue;let [,id,label,status]=line.split('|');steps.set(id,{label,status})}let target=document.querySelector('#precheck-steps');if(!steps.size){target.className='muted';target.textContent='Waiting for precheck progress…';return}target.className='';target.innerHTML=[...steps.values()].map(s=>{let done=s.status==='done',value=done?100:55;return `<div class="step ${done?'step-done':'step-running'}"><div class="step-head"><span>${esc(s.label)}</span><span>${done?'Complete':'Working…'}</span></div><progress value="${value}" max="100"></progress></div>`}).join('')}
