...
#neural-core{width:210px;height:210px;border:1px solid #4c78aa;border-radius:12px;background:#070e1b;box-shadow:inset 0 0 40px rgba(82,199,139,.25),0 0 30px rgba(82,199,139,.14)}.precheck-panel{display:grid;grid-template-columns:minmax(0,1fr) 210px;gap:18px;align-items:stretch}.overview-tools{display:flex;align-items:center;gap:10px}.overview-title h2{margin:0}@media(max-width:1200px){.precheck-panel{grid-template-columns:1fr}.precheck-panel #neural-core{display:none}}
...
<aside><section class="precheck-panel"><div><div class="overview-title"><h2>Precheck overview</h2><div class="overview-tools"><span id="precheck-status" class="badge">Not run</span></div></div><div id="precheck-steps" class="muted">Run a precheck to see its progress.</div></div><canvas id="neural-core" width="210" height="210" aria-label="Live neural feedback"></canvas></section><section><h2>Technical log</h2><pre id="log">Select a precheck to view its log.</pre></section></aside></main>
<script>
...
const neuralCanvas=document.querySelector('#neural-core'),neuralContext=neuralCanvas.getContext('2d'),neuralNodes=[[27,43],[77,24],[145,35],[183,69],[45,111],[103,89],[165,120],[73,174],[132,178],[187,166]],neuralEdges=[[0,1],[1,2],[2,3],[0,4],[1,5],[2,5],[3,6],[4,5],[5,6],[4,7],[5,7],[5,8],[6,8],[6,9],[7,8],[8,9]];
function drawNeuralCore(time=0){let c=neuralContext,raw=Math.max(Number(scan.cpu_load_percent)||0,100-(Number(scan.total_ram_gib)?Number(scan.available_ram_gib)/Number(scan.total_ram_gib)*100:100),Number(scan.total_vram_gib)?100-Number(scan.available_vram_gib)/Number(scan.total_vram_gib)*100:0)/100,load=.16+raw*.84,center=105,nodes=neuralNodes.map(([x,y],i)=>{let expansion=1+raw*.3+Math.sin(time/800+i)*.025;return [center+(x-center)*expansion+Math.sin(time/650+i*2.3)*3*load,center+(y-center)*expansion+Math.cos(time/780+i*1.7)*3*load]});c.clearRect(0,0,210,210);c.fillStyle='#070e1b';c.fillRect(0,0,210,210);c.strokeStyle='rgba(82,199,139,.1)';for(let x=0;x<210;x+=21){c.beginPath();c.moveTo(x,0);c.lineTo(x,210);c.stroke()}for(let y=0;y<210;y+=21){c.beginPath();c.moveTo(0,y);c.lineTo(210,y);c.stroke()}neuralEdges.forEach(([a,b],i)=>{let p=nodes[a],q=nodes[b],speed=.35+load*2,t=(time/1000*speed+i*.19)%1,x=p[0]+(q[0]-p[0])*t,y=p[1]+(q[1]-p[1])*t;c.strokeStyle=`rgba(82,199,139,${.18+load*.38})`;c.lineWidth=1.2;c.beginPath();c.moveTo(p[0],p[1]);c.lineTo(q[0],q[1]);c.stroke();let g=c.createRadialGradient(x,y,0,x,y,8+load*7);g.addColorStop(0,'rgba(220,255,240,1)');g.addColorStop(.25,'rgba(82,199,139,.95)');g.addColorStop(1,'rgba(82,199,139,0)');c.fillStyle=g;c.beginPath();c.arc(x,y,8+load*7,0,Math.PI*2);c.fill()});nodes.forEach(([x,y],i)=>{let pulse=(Math.sin(time/(300+raw*180)+i*1.7)+1)/2,r=3+load*4+pulse*2,g=c.createRadialGradient(x,y,0,x,y,r*3);g.addColorStop(0,'rgba(225,255,242,1)');g.addColorStop(.28,'rgba(82,199,139,.95)');g.addColorStop(1,'rgba(82,199,139,0)');c.fillStyle=g;c.beginPath();c.arc(x,y,r*3,0,Math.PI*2);c.fill()});c.fillStyle='#caffe1';c.font='bold 13px system-ui';c.fillText(`NEURAL LOAD  ${Math.round(raw*100)}%`,25,195);requestAnimationFrame(drawNeuralCore)}
