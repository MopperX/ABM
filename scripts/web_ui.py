...
#neural-core{width:96px;height:96px;border:1px solid #4c78aa;border-radius:10px;background:#070e1b;box-shadow:inset 0 0 28px rgba(82,199,139,.25),0 0 28px rgba(82,199,139,.18)}.overview-tools{display:flex;align-items:center;gap:12px}.overview-title h2{margin:0}
...
<aside><section><div class="overview-title"><h2>Precheck overview</h2><div class="overview-tools"><span id="precheck-status" class="badge">Not run</span><canvas id="neural-core" width="124" height="124" aria-label="Live neural feedback"></canvas></div></div><div id="precheck-steps" class="muted">Run a precheck to see its progress.</div></section><section><h2>Technical log</h2><pre id="log">Select a precheck to view its log.</pre></section></aside></main>
<script>
...
const neuralCanvas=document.querySelector('#neural-core'),neuralContext=neuralCanvas.getContext('2d'),neuralNodes=[[18,27],[48,16],[88,27],[27,65],[64,53],[103,70],[42,101],[83,99]];
function drawNeuralCore(time=0){let c=neuralContext,load=Math.max(Number(scan.cpu_load_percent)||0,100-(Number(scan.total_ram_gib)?Number(scan.available_ram_gib)/Number(scan.total_ram_gib)*100:100),Number(scan.total_vram_gib)?100-Number(scan.available_vram_gib)/Number(scan.total_vram_gib)*100:0)/100,pulse=(Math.sin(time/420)+1)/2;c.clearRect(0,0,124,124);c.fillStyle='#0b1220';c.fillRect(0,0,124,124);for(let [a,b] of [[0,1],[1,2],[0,3],[1,4],[2,4],[2,5],[3,4],[3,6],[4,5],[4,6],[4,7],[5,7],[6,7]]){let p=neuralNodes[a],q=neuralNodes[b];c.strokeStyle=`rgba(82,199,139,${.15+load*.36})`;c.lineWidth=1;c.beginPath();c.moveTo(p[0],p[1]);c.lineTo(q[0],q[1]);c.stroke()}neuralNodes.forEach((p,i)=>{let r=2.3+(i%3===0?pulse*2.6:load*2),g=c.createRadialGradient(p[0],p[1],0,p[0],p[1],10+r);g.addColorStop(0,`rgba(160,255,213,${.9})`);g.addColorStop(.25,`rgba(82,199,139,${.7})`);g.addColorStop(1,'rgba(82,199,139,0)');c.fillStyle=g;c.beginPath();c.arc(p[0],p[1],10+r,0,Math.PI*2);c.fill()});c.fillStyle='#d6fff0';c.font='bold 10px system-ui';c.fillText(`${Math.round(load*100)}%`,47,118);requestAnimationFrame(drawNeuralCore)}
