...
function resourceCards(){let ram=Number(scan.total_ram_gib)||0,free=Number(scan.available_ram_gib)||0,vram=Number(scan.total_vram_gib)||0,vfree=Number(scan.available_vram_gib)||0,disk=storage=>{let total=Number(storage?.total_gib)||0,available=Number(storage?.free_gib)||0,used=Number(storage?.used_gib)||0,percent=total?Math.round(available/total*100):0;return `<div class="metric"><span class="muted">${esc(storage?.label||'Disk')}</span><b>${percent}% vrij</b><small>${available.toFixed(2)} GiB vrij / ${total.toFixed(2)} GiB totaal (${used.toFixed(2)} GiB gebruikt)</small></div>`},root={...(scan.root_storage||{}),label:'Root disk'},ollama={...(scan.ollama_models_storage||{}),label:'Ollama disk'},disks=scan.ollama_models_on_root_filesystem?disk({...ollama,label:'Root & Ollama disk'}):disk(root)+disk(ollama);return `<div class="metric"><span class="muted">CPU</span><b>${Number(scan.cpu_load_percent||0).toFixed(0)}%</b><small>${esc(scan.cpu||'Unknown')}</small></div><div class="metric"><span class="muted">RAM</span><b>${free.toFixed(1)} GiB vrij</b><small>${ram.toFixed(1)} GiB totaal</small></div><div class="metric"><span class="muted">GPU</span><b>${vram.toFixed(1)} GiB VRAM</b><small>${esc(scan.gpu||'Geen GPU')}</small></div><div class="metric"><span class="muted">VRAM</span><b>${vfree.toFixed(1)} GiB vrij</b><small>${vram.toFixed(1)} GiB totaal</small></div>${disks}`}
