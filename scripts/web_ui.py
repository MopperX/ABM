...
def tail(path: Path, size: int | None = None) -> str:
    try:
...
function setLog(text){let log=document.querySelector('#log');log.textContent=text;if(followLog)log.scrollTop=log.scrollHeight}
...
function renderPrecheckSteps(output){let events=new Map;for(let line of output.split('\n')){if(!line.startsWith('@@PRECHECK|'))continue;let [,id,label,status]=line.split('|');events.set(id,{label,status})}let target=document.querySelector('#precheck-steps'),rows=[];function row(label,detail,value,done){let percent=Math.max(0,Math.min(100,Math.round(value)));rows.push(`<div class="step ${done?'step-done':'step-running'}"><div class="step-head"><span>${esc(label)}</span><span>${done?'Complete':percent+'%'}</span></div><span class="step-detail">${esc(detail)}</span><progress value="${percent}" max="100"></progress></div>`)}let system=events.get('environment');if(system)row('System readiness','Checking disk space, required tools and the Ollama service.',system.status==='done'?100:50,system.status==='done');let suites=(precheck?.suites||[]).map(s=>({id:'suite-'+s,name:(suiteLabels[s]||[s])[0]})),completedSuites=suites.filter(s=>events.get(s.id)?.status==='done').length,activeSuite=suites.find(s=>events.get(s.id)?.status==='running');if(suites.length)row('Preparing benchmark suites',activeSuite?activeSuite.name+' is being prepared.':completedSuites===suites.length?'All selected suites are ready.':`${completedSuites} of ${suites.length} suites prepared.`,completedSuites/suites.length*100,completedSuites===suites.length);let models=precheck?.models||[],modelEvents=models.map(m=>({model:m,event:events.get('model-'+m)})),completedModels=modelEvents.filter(x=>x.event?.status==='done').length,activeModel=modelEvents.find(x=>x.event?.status==='running'),progressMatches=[...output.matchAll(/pulling\s+[^:\s]+:\s+(\d+)%/g)],downloadPercent=progressMatches.length?Number(progressMatches.at(-1)[1]):0;if(models.length){let value=(completedModels+(activeModel?downloadPercent/100:0))/models.length*100,detail=activeModel?`Downloading ${activeModel.model} — ${downloadPercent}%` : completedModels===models.length?'All selected models are ready.':`Waiting to download ${models.length-completedModels} model(s).`;row('Downloading models',detail,value,completedModels===models.length)}if(!rows.length){target.className='muted';target.textContent='Waiting for precheck progress…';return}target.className='';target.innerHTML=rows.join('')}
...
function invalidatePrecheck(){precheck=null;document.querySelector('#start').disabled=true;document.querySelector('#stop-precheck').disabled=true;document.querySelector('#precheck-report').className='precheck';document.querySelector('#precheck-report').textContent='Run the precheck before starting a benchmark.'}
...
class App(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        if self.path not in {"/api/start", "/api/precheck", "/api/precheck-status", "/api/precheck-stop"}: self.send_json({"error": "Not found"}, 404); return
        try:
