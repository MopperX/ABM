...
const extraStyle=document.createElement('style');extraStyle.textContent='.metrics{display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:8px;margin:0 0 16px}.metric{background:#101827;border:1px solid #2c3b55;border-radius:8px;padding:10px}.metric b{display:block;font-size:17px}.metric small{display:block;color:#aebbd1;font-size:11px;margin-top:4px}.space-green{color:#52c78b}.space-orange{color:#ffb454}.space-red{color:#ff6b6b}@media(max-width:850px){.metrics{grid-template-columns:repeat(2,1fr)}}';document.head.append(extraStyle);
function resourceCards(){let colour=value=>value<15?'space-red':value<30?'space-orange':'space-green',ram=Number(scan.total_ram_gib)||0,free=Number(scan.available_ram_gib)||0,vram=Number(scan.total_vram_gib)||0,vfree=Number(scan.available_vram_gib)||0,memory=(label,available,total,empty='Geen GPU')=>{if(!total)return `<div class="metric"><span class="muted">${label}</span><b>0 GiB</b><small>${empty}</small></div>`;let percent=Math.round(available/total*100),used=total-available;return `<div class="metric"><span class="muted">${label}</span><b class="${colour(percent)}">${percent}% vrij</b><small>${available.toFixed(2)} GiB vrij / ${total.toFixed(2)} GiB totaal (${used.toFixed(2)} GiB gebruikt)</small></div>`},disk=(label,storage)=>{let total=Number(storage?.total_gib)||0,available=Number(storage?.free_gib)||0,used=Number(storage?.used_gib)||0,percent=total?Math.round(available/total*100):0;return `<div class="metric"><span class="muted">${label}</span><b class="${colour(percent)}">${percent}% vrij</b><small>${available.toFixed(2)} GiB vrij / ${total.toFixed(2)} GiB totaal (${used.toFixed(2)} GiB gebruikt)</small></div>`},root=scan.root_storage||{},ollama=scan.ollama_models_storage||{},disks=scan.ollama_models_on_root_filesystem?disk('Root & Ollama disk',ollama):disk('Root disk',root)+disk('Ollama disk',ollama),load=Number(scan.cpu_load_percent)||0,cpuColour=load>85?'space-red':load>60?'space-orange':'space-green';return `<div class="metric"><span class="muted">CPU (1 min load)</span><b class="${cpuColour}">${load.toFixed(0)}%</b><small>${esc(scan.cpu||'Unknown CPU')} · ${scan.cpu_count??'?'} logical cores</small></div>${memory('Available RAM',free,ram,'No RAM reported.') }<div class="metric"><span class="muted">GPU</span><b>${vram?`${vram.toFixed(1)} GiB VRAM`:'0 GiB VRAM'}</b><small>${esc(scan.gpu||'Geen GPU')}</small></div>${memory('Free VRAM',vfree,vram)}${disks}`}
