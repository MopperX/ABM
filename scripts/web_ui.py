...
function renderPrecheckSteps(output){let events=new Map;for(let line of output.split('\n')){if(line.startsWith('@@PRECHECK|')){let [,id,label,status]=line.split('|');events.set(id,{label,status})}}let rows=[];for(let [id,event] of events){let done=event.status==='done';rows.push(`<div class="step ${done?'step-done':'step-running'}"><div class="step-head"><span>${esc(event.label||id)}</span><span>${done?'Voltooid':'Bezig'}</span></div><progress value="${done?100:40}" max="100"></progress></div>`)}let target=document.querySelector('#precheck-steps');target.innerHTML=rows.length?rows.join(''):'<span class="muted">Wachten op voortgang…</span>'}
