...
#neural-core{display:none}#force-graph{flex:1;min-height:0;border-radius:11px;overflow:hidden;background:#182235}.neural-panel{display:flex;flex-direction:column;min-height:0;padding:0!important;overflow:hidden}.overview-tools{display:flex;align-items:center;gap:10px}.overview-title h2{margin:0}
...
const neuralFillRect=neuralContext.fillRect.bind(neuralContext);neuralContext.fillRect=(x,y,width,height)=>{if(neuralContext.fillStyle==='#030b16')neuralContext.fillStyle='#182235';neuralFillRect(x,y,width,height)};neuralContext.fillText=()=>{};neuralCanvas.height=340;setInterval(morphNeuralDynamics,40);setInterval(updateNeuralPresence,40);requestAnimationFrame(drawStableResourceNetwork);load().then(updateForceGraph).catch(e=>document.body.insertAdjacentHTML('beforeend','<pre>'+esc(e.message)+'</pre>'));setInterval(()=>{refreshResources();updateForceGraph()},10000)
</script></html>"""
