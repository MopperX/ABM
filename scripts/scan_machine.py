...
def main() -> int:
    ...
    disk_gib = shutil.disk_usage(Path(args.output).resolve().parent).free / GIB
    ...
    for raw, row in zip(catalog.get("models", []), parse_model_rows(models_path, enabled_only=False)):
        ...
        disk_ok = disk_gib >= required_disk
        ...
        result = {"model": row["model"], "allowed": allowed, "eligibility": eligibility, "requirements": {"ram_gib": required_ram, "vram_gib": required_vram, "free_disk_gib": required_disk}}
        if allowed:
            ...
            if not disk_ok:
                reasons.append(f"requires {required_disk:g} GiB free disk; {disk_gib:.2f} GiB available")
    ...
    report = {
        ...
        "available_disk_gib": round(disk_gib, 2),
        "models": findings,
    }
