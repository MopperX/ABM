...
Use `--cleanup-on-success` to discard every installed Ollama model except the models named with `--keep-model`, as well as the complete reusable benchmark cache (datasets, evaluators, and non-Ollama model downloads). Cleanup runs only when the benchmark has fully completed; failed or stopped runs retain all assets so they can be inspected or resumed. Run evidence in `BENCH_RESULTS_DIR/runs` is always retained. Because this removes all non-retained Ollama models, list every Ollama model you want to preserve:
...
The cleanup report is written to `summary/cleanup.json`. To clean an already completed run, use `./benchmark cleanup <run-id>`. It reads that run's frozen machine scan and asks which Ollama models to retain; it then removes the remaining Ollama models and the complete reusable benchmark cache. Run evidence remains untouched. This interactive command is the recommended cleanup workflow before repurposing a machine.
